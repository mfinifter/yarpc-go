...
// of requests and responses for the Store service.
...
func (s *decoder) DecodeRequest(procedure string, requestBody wire.Value) (interface{}, error) {
	...
	case "Store::Forget":
		...
		if err := args.FromWire(requestBody); err != nil {
			return nil, yarpcerrors.InvalidArgumentErrorf(
				"could not decode Thrift request for service 'Store procedure 'Forget': %w", err)
		}
		return args, nil
	...
	default:
		return nil, yarpcerrors.InvalidArgumentErrorf(
			"could not decode Thrift request for service 'Store' procedure '%s': unknown procedure", procedure)
	}
}
...
func (s *decoder) DecodeResponse(procedure string, responseBody wire.Value) (interface{}, error) {
	...
	case "Store::CompareAndSwap":
		...
		if err := result.FromWire(responseBody); err != nil {
			return nil, yarpcerrors.InvalidArgumentErrorf(
				"could not decode Thrift response for service 'Store procedure 'CompareAndSwap': %w", err)
		}
		return result, nil
	...
	case "Store::Increment":
		...
		if err := result.FromWire(responseBody); err != nil {
			return nil, yarpcerrors.InvalidArgumentErrorf(
				"could not decode Thrift response for service 'Store procedure 'Increment': %w", err)
		}
		return result, nil

	default:
		return nil, yarpcerrors.InvalidArgumentErrorf(
			"could not decode Thrift response for service 'Store' procedure '%s': unknown procedure", procedure)
	}
}
