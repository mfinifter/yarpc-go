...
// of requests and responses for the BaseService service.
...
func (s *decoder) DecodeRequest(procedure string, requestBody wire.Value) (interface{}, error) {
	...
	default:
		return nil, yarpcerrors.InvalidArgumentErrorf(
			"could not decode Thrift request for service 'BaseService' procedure '%s': unknown procedure", procedure)
	}
}
...
func (s *decoder) DecodeResponse(procedure string, responseBody wire.Value) (interface{}, error) {
	...
	case "BaseService::Healthy":
		...
		if err := result.FromWire(responseBody); err != nil {
			return nil, yarpcerrors.InvalidArgumentErrorf(
				"could not decode Thrift response for service 'BaseService procedure 'Healthy': %w", err)
		}
		return result, nil

	default:
		return nil, yarpcerrors.InvalidArgumentErrorf(
			"could not decode Thrift response for service 'BaseService' procedure '%s': unknown procedure", procedure)
	}
}
